<template>
  <div class="flex flex-col gap-12 text-sm">
    <header class="rounded-sm overflow-hidden bg-dill-100">
      <h1 class="flex flex-col gap-y-2 px-6 py-4">
        <span class="font-display text-rot-600 text-4xl">Jacob Hardbower</span>
        <span>is a designer and developer of the web</span>
      </h1>
      <div class="bg-dill-800 p-2 flex gap-2 text-dill-200 text-xs">
    ...
    <main class="flex flex-col gap-16">
      ...
      >
        <div class="flex flex-col gap-6">
          ...
          />
          <h2 class="font-display text-rot-600 text-4xl">{{ name }}</h2>
          <p v-for="paragraph in description" :key="paragraph">
            {{ paragraph }}
          </p>
        </div>
        <div class="bg-dill-100 rounded-sm">
